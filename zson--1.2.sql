...
BEGIN
    ...
    LOOP
        ...
        query := query || '( select unnest(zson_extract_strings(' ||
                    quote_ident(colname) || ')) as t from ' || tabname || ' tablesample system( (1000 * 100)::decimal / coalesce(nullif(select reltuples::bigint from pg_class where pg_catalog.relname=' || quote_ident(relname) || '), 0), 1000) order by random() limit ' ||
                    max_examples || ')';
